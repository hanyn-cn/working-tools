...
def calculate_bonus(dir_docs, docs, rest_docs, new_docs,
    ...
    # 主任医师奖金（固定每人同样奖金，显示名改为“主任”）
    dir_results = []
    sum_dir_doc = 0
    for ddoc in dir_docs:
        dir_results.append({"姓名": "主任", "奖金": cheng_tot + 11})
        sum_dir_doc += cheng_tot + 11

    # 李珊珊奖金改名为“前台0”奖金
    # 原来：lishanshan = hospital_pingjun + 2000 + 11
    # 改为：
    qiantai0 = hospital_pingjun + 2000 + 11

    # 实发奖金总数
    shifa = sum_doc + sum_new_doc + sum_rest_doc + sum_dir_doc + 600 + qiantai0 + jiangli

    # 汇总所有奖金详情表
    all_results = normal_results + rest_results + new_results + dir_results
    # 替换奖金明细里“李珊珊”为“前台0”，但李珊珊奖金单独计算，不在表里，所以无须改表格
...
if "dir_docs" not in st.session_state:
    # 这里存储原来的程伟波，但显示改名为主任，不影响计算，只显示时用“主任”
    st.session_state.dir_docs = [{"姓名": "程伟波", "报告数": 0, "工作量": 0}]

if st.session_state.page == "home":
    st.title("奖金分配工具首页")

    # 首页文本内容可编辑保存到session_state，默认一段欢迎文本
    if "home_text" not in st.session_state:
        st.session_state.home_text = (
            "欢迎使用奖金分配工具！\n\n"
            "您可以在此编辑首页显示的介绍文字，方便以后修改。\n\n"
            "点击下方按钮开始使用。"
        )
    home_text = st.text_area("首页介绍文字（可编辑）", value=st.session_state.home_text, height=200)
    st.session_state.home_text = home_text

    if st.button("开始使用"):
        st.session_state.page = "input_basic"
        st.experimental_rerun()

elif st.session_state.page == "input_basic":
